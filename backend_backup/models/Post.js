...
postSchema.methods.calculateTrendingScore = function() {
  ...
  // Ensure reactionCounts.total is a valid number
  const totalReactions = (this.reactionCounts && typeof this.reactionCounts.total === 'number' && !isNaN(this.reactionCounts.total)) 
    ? this.reactionCounts.total 
    : 0;
  
  const reactionWeight = totalReactions * 2;
  const commentWeight = (this.comments && this.comments.length) ? this.comments.length * 3 : 0;
  const timeDecay = Math.pow(0.9, Math.max(0, ageInHours)); // Ensure non-negative
  
  const score = (reactionWeight + commentWeight) * timeDecay;
  this.trending.score = isNaN(score) ? 0 : score;
...
postSchema.methods.addReaction = async function(userId, reactionType) {
  ...
  // Ensure reaction array exists before pushing
  ...
  // Update counts - ensure all reaction arrays exist
  const funnyCount = (this.reactions.funny && Array.isArray(this.reactions.funny)) ? this.reactions.funny.length : 0;
  const rageCount = (this.reactions.rage && Array.isArray(this.reactions.rage)) ? this.reactions.rage.length : 0;
  const shockCount = (this.reactions.shock && Array.isArray(this.reactions.shock)) ? this.reactions.shock.length : 0;
  const relatableCount = (this.reactions.relatable && Array.isArray(this.reactions.relatable)) ? this.reactions.relatable.length : 0;
  const loveCount = (this.reactions.love && Array.isArray(this.reactions.love)) ? this.reactions.love.length : 0;
  const thinkingCount = (this.reactions.thinking && Array.isArray(this.reactions.thinking)) ? this.reactions.thinking.length : 0;
  ...
  this.reactionCounts = {
    ...
    total: funnyCount + rageCount + shockCount + relatableCount + loveCount + thinkingCount
  };
